...
2. Pick a branch name as `<type>/<slug>` (Jira is disabled — no `<JIRA-KEY>` in branch names; see `docs/jira-integration-disabled.md`). `type` is a Conventional-Commit-style prefix (`feat`, `fix`, `chore`, `docs`, `refactor`, `test`) matching the change; `slug` is a short kebab-case description (3-5 words) inferred from the task or the diff.
3. Create and switch to it: `git checkout -b <branch-name>`.
4. Jira `start-story` is skipped: Jira is disabled (see `docs/jira-integration-disabled.md`). Do not run `python -m jira_sync_kit ...`.
