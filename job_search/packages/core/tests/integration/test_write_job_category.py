...
class TestWriteJobCategory(unittest.TestCase):
    ...
    def test_classifies_and_writes_a_real_job_via_rules_stage(self) -> None:
        ...
        with self.engine.connect() as conn:
            row = conn.execute(
                text(
                    "SELECT category, category_method, seniority_band, "
                    "qa_category, prompt_version, model_id "
        ...
        # A rules-resolved row never called an LLM, so the _UPSERT's
        # prompt_version/model_id columns must be NULL for it.
...
class _FixedLLMAdapter:
    """A stub `LLMAdapter` that always returns the same, valid
    classification JSON — no real Anthropic call needed.
    """

    def __init__(self, category: str = "other", confidence: float = 0.87) -> None:
        """Initialise the fake adapter.

        Args:
            category: The category this adapter's `complete` response
                will name.
            confidence: The confidence this adapter's `complete`
                response will name.
        """
    ...
    def complete(
        ...
        *,
        ...
        """Return a canned, valid classification response.

        Args:
            model: The provider-specific model identifier (echoed back
                unused, to satisfy `LLMAdapter`'s interface).
            prompt: The rendered classification prompt (unused — this
                adapter always returns the same fixed response).
            temperature: Unused — accepted only to satisfy `LLMAdapter`.
            seed: Unused — accepted only to satisfy `LLMAdapter`.

        Returns:
            An `LLMResponse` whose `text` is a JSON object naming this
            adapter's configured category and confidence.
        """
        return LLMResponse(
            text=json.dumps(
...
@unittest.skipUnless(_ollama_available(), "Ollama server not reachable")
class TestWriteJobCategoryLlmStagePersistence(unittest.TestCase):
    """Deterministic, unconditional coverage of the `_UPSERT`'s
    prompt_version/model_id columns for an LLM-classified row.

    Unlike `TestWriteJobCategory` (gated behind a live
    `ANTHROPIC_API_KEY`), this class fakes the LLM call via
    `_FixedLLMAdapter` and so runs in every environment — the previous
    `test_llm_classified_row_carries_prompt_version_and_model_id`
    passed regardless of which branch its fixture actually took,
    giving zero real coverage of the LLM-classified path (JOB-182 final
    review, finding 4). "Chief Ethics Officer" is the same
    no-rules/no-embedding-match title `test_classification_llm.py`
    uses for exactly this reason — it reliably cascades all the way to
    the LLM stage. The embedding stage itself still makes a real call
    to a local Ollama server (not faked), so this class is skipped when
    Ollama isn't reachable, same as `TestWriteJobCategory`.
    """

    def setUp(self) -> None:
        """Insert one fixture job (title guaranteed to reach the LLM
        stage) into the silver tables `write_job_category` reads.
        ...
        self.http_client = httpx.Client(timeout=30.0)
        self.suffix = uuid.uuid4().hex
        self.job_key = f"gh-llm-{self.suffix}"
        self.job_group_id = f"group-llm-{self.suffix}"
        with self.engine.begin() as conn:
            conn.execute(
                text(
                    "INSERT INTO silver.silver__job_posting "
                    ...
                    "location, description, salary_raw, posted_at) "
                    "VALUES (:job_key, 'greenhouse', :source_job_id, "
                    "'https://greenhouse.example/cat-llm-test', "
                    "'https://greenhouse.example/cat-llm-test', 'api', "
                    "'Chief Ethics Officer', 'Test Co', 'London', "
                    "'A description', NULL, now())"
                ),
                {"job_key": self.job_key, "source_job_id": f"src-llm-{self.suffix}"},
            )
            conn.execute(
                text(
                    "INSERT INTO silver.job_identity_map "
                ...
                {
                    "source_job_id": f"src-llm-{self.suffix}",
                    "job_group_id": self.job_group_id,
                },
            )
            conn.execute(
                text(
                    ...
                    "apply_source_job_id, apply_job_url, apply_title_for_display) "
                    "VALUES (:job_group_id, 'A description', 'greenhouse', "
                    ":source_job_id, 'https://greenhouse.example/cat-llm-test', "
    ...
    def tearDown(self) -> None:
        """Delete this test's fixture rows.
    ...
    def test_llm_classified_row_gets_real_prompt_version_and_model_id(self) -> None:
        """An LLM-classified row must persist non-NULL
        prompt_version/model_id — a real assertion that would fail if
        the `_UPSERT` regressed to writing NULL (or the wrong value)
        for these columns.
        ...
        Raises:
            AssertionError: If the written row isn't "llm"-method, or
                its prompt_version/model_id are NULL or wrong.
        """
        adapters = {"anthropic": _FixedLLMAdapter(category="other", confidence=0.87)}
        written = write_job_category(
            self.engine,
            adapters=adapters,
            http_client=self.http_client,
        ...
        with self.engine.connect() as conn:
            row = conn.execute(
                text(
                    "SELECT category, category_method, prompt_version, model_id "
                    "FROM silver.job_category WHERE job_group_id = :g"
                ),
                {"g": self.job_group_id},
            ).one()
        self.assertEqual(row.category, "other")
        self.assertEqual(row.category_method, "llm")
        self.assertIsNotNone(row.prompt_version)
        self.assertIsNotNone(row.model_id)
