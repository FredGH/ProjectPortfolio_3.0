...
class TestSaveToParquet:
    ...
    def test_save_simple_dict(self, tmp_path, monkeypatch):
        ...
        data_zone = tmp_path / "data_zone"
        data_zone.mkdir()
        monkeypatch.setattr("open_weather_sources.extraction.DATA_ZONE_PATH", data_zone)
        
        data = {"id": 1, "name": "Test"}
        filepath = save_to_parquet(data, "test_source", "20240315_120000")
        
        assert filepath.exists()
        assert filepath.name == "test_source_20240315_120000.parquet"
...
class TestDataZoneFiles:
    ...
    def test_list_data_zone_files(self, tmp_path, monkeypatch):
        ...
        pq.write_table(pa.Table.from_pandas(df), str(data_zone / "source1_20240315_120000.parquet"))
        pq.write_table(pa.Table.from_pandas(df), str(data_zone / "source2_20240315_120000.parquet"))
        
        from open_weather_sources.extraction import list_data_zone_files
