...
def generate_trader_digest(week_ending: date) -> Path:
    """Trader attribution table for the week, written to CSV."""
    week_start = week_ending - timedelta(days=4)

    sql = sa.text(
        "SELECT trader_id, instrument_class, "
        ...
        "ROUND(SUM(quantity * avg_fill_price)::numeric / 1e6, 2) AS total_notional_eur_m "
        "FROM mart_trading_risk.fact_order_execution "
        "WHERE trade_date BETWEEN :s AND :e "
        "GROUP BY trader_id, instrument_class "
        "ORDER BY avg_arrival_slippage_bps DESC"
    )
