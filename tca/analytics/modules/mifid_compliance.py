...
class MifidCompliance:
    ...
    _REQUIRED_FIELDS = ["counterparty_id", "venue_id", "instrument_id", "trade_date", "side"]
    ...
    def run(
        ...
        rts27 = (
            ...
            .agg(
                ...
                avg_slippage_bps=("arrival_slippage_bps", "mean") if "arrival_slippage_bps" in df.columns else ("hub_order_key", "count"),
                fill_rate_pct=("fill_rate_pct", "mean") if "fill_rate_pct" in df.columns else ("hub_order_key", "count"),
                compliant_order_count=("is_compliant", "sum"),
            )
            .round(4)
            .reset_index()
        )
        rts27["rts27_quality_flag"] = rts27["compliant_order_count"] == rts27["order_count"]
