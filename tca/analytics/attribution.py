...
def decompose(orders: pd.DataFrame, fills: pd.DataFrame) -> pd.DataFrame:
    ...
    df["trader_cost_bps"] = (
        df["avg_commission_bps"].fillna(0)
        + (df["arrival_slippage_bps"].fillna(0) - df["algo_cost_bps"] - df["market_cost_bps"]).clip(lower=0)
    )
    ...
    result["pct_market"] = (
        result["avg_market_cost_bps"] / result["avg_total_cost_bps"].replace(0, float("nan")) * 100
    ).round(1)
    result["pct_algo"] = (
        result["avg_algo_cost_bps"] / result["avg_total_cost_bps"].replace(0, float("nan")) * 100
    ).round(1)
    result["pct_trader"] = (100 - result["pct_market"].fillna(0) - result["pct_algo"].fillna(0)).round(1)
